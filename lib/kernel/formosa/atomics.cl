...
#define FORMOSA_DEF_AMO_HELPER_AS(TYPE, NAME, ASM, AS)                         \
  static inline TYPE _CL_OVERLOADABLE formosa_##NAME(volatile AS TYPE *p,      \
                                                     TYPE val) {               \
    TYPE old;                                                                  \
...
#define FORMOSA_DEF_NAMED_BIN_AS(FN, TYPE, AS, BODY)                           \
  __attribute__((overloadable)) TYPE FN(volatile AS TYPE *p, TYPE val) { BODY }
...
FORMOSA_DEF_NAMED_BIN(_cl_atomic_min, uint, return formosa_amominu_u32(p, val);)
FORMOSA_DEF_NAMED_BIN(_cl_atomic_max, int, return formosa_amomax_i32(p, val);)
FORMOSA_DEF_NAMED_BIN(_cl_atomic_max, uint, return formosa_amomaxu_u32(p, val);)

#ifdef cl_khr_int64_base_atomics
FORMOSA_DEF_NAMED_BIN(_cl_atom_min, long, return formosa_amomin_i64(p, val);)
FORMOSA_DEF_NAMED_BIN(_cl_atom_min, ulong, return formosa_amominu_u64(p, val);)
FORMOSA_DEF_NAMED_BIN(_cl_atom_max, long, return formosa_amomax_i64(p, val);)
FORMOSA_DEF_NAMED_BIN(_cl_atom_max, ulong, return formosa_amomaxu_u64(p, val);)
#endif

__attribute__((overloadable)) float atomic_xchg(volatile __global float *p,
                                                float val) {
  return as_float(
      formosa_amoswap_u32((volatile __global uint *)p, as_uint(val)));
}

__attribute__((overloadable)) float atomic_xchg(volatile __local float *p,
                                                float val) {
  return as_float(
      formosa_amoswap_u32((volatile __local uint *)p, as_uint(val)));
}

__attribute__((overloadable)) float atom_xchg(volatile __global float *p,
                                              float val) {
  return atomic_xchg(p, val);
}

__attribute__((overloadable)) float atom_xchg(volatile __local float *p,
                                              float val) {
  return atomic_xchg(p, val);
}

#ifdef cl_khr_int64_base_atomics
__attribute__((overloadable)) double atomic_xchg(volatile __global double *p,
                                                 double val) {
  return as_double(
      formosa_amoswap_u64((volatile __global ulong *)p, as_ulong(val)));
}

__attribute__((overloadable)) double atomic_xchg(volatile __local double *p,
                                                 double val) {
  return as_double(
      formosa_amoswap_u64((volatile __local ulong *)p, as_ulong(val)));
}

__attribute__((overloadable)) double atom_xchg(volatile __global double *p,
                                               double val) {
  return atomic_xchg(p, val);
}

__attribute__((overloadable)) double atom_xchg(volatile __local double *p,
                                               double val) {
  return atomic_xchg(p, val);
}
#endif
